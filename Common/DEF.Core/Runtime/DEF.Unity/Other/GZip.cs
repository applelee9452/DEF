...
public class GZip
{
    ...
    {
        if (!Directory.Exists(lpSourceFolder))
        {
            throw new Exception("压缩文件目录不能为空！lpSourceFolder" + lpSourceFolder);
        }

        // 解压文件夹为空时默认与压缩文件同一级目录下，跟压缩文件同名的文件夹  
        if (string.IsNullOrEmpty(lpDestFolder))
        {
            lpDestFolder = lpSourceFolder;
        }

        lpSourceFolder = lpSourceFolder.Replace(@"\", "/");
        lpDestFolder = lpDestFolder.Replace(@"\", "/");

        if (!lpSourceFolder.EndsWith("/"))
        {
            lpSourceFolder += "/";
        }
    ...
    private static GZipResult Compress(string lpSourceFolder, string searchPattern, SearchOption searchOption, string lpDestFolder, string zipFileName, bool deleteTempFile)
    {
    ...
    /// <param name="lpDestFolder">Folder to write the zip file into</param>
    /// <param name="zipFileName">Name of the zip file to write</param>
    private static GZipResult Compress(FileInfo[] files, string[] folders, string lpBaseFolder, string lpDestFolder, string zipFileName)
    {
    ...
    /// <param name="zipFileName">Name of the zip file to write</param>
    /// <param name="deleteTempFile">Boolean, true deleted the intermediate temp file, false leaves the temp file in lpDestFolder (for debugging)</param>
    private static GZipResult Compress(FileInfo[] files, string lpBaseFolder, string lpDestFolder, string zipFileName, bool deleteTempFile)
    {
    ...
    private static void CreateTempFile(FileInfo[] files, string lpBaseFolder, string lpTempFile, GZipResult result)
    {
        ...
        string fileModDate = string.Empty;
        int fileIndex = 0;
        string lpSourceFile = string.Empty;
        string vpSourceFile = string.Empty;
            ...
            try
            {
                ...
                foreach (FileInfo fi in files)
                {
                    try
                    {
                        ...
                        lpSourceFile = lpSourceFile.Replace(@"\", "/");
    ...
    public static GZipResult Decompress(string lpSourceFolder, string lpDestFolder, string zipFileName)
    {
        if (!Directory.Exists(lpSourceFolder))
        {
            throw new Exception("解压文件目录不能为空！lpSourceFolder=" + lpSourceFolder);
        }

        lpSourceFolder = lpSourceFolder.Replace(@"\", "/");
        lpDestFolder = lpDestFolder.Replace(@"\", "/");

        string zip_file_path = Path.Combine(lpSourceFolder, zipFileName);
        if (!File.Exists(zip_file_path))
        {
            throw new FileNotFoundException("解压文件不存在！zip_file_path=" + zip_file_path);
        }
    ...
    private static GZipResult Decompress(string lpSrcFolder, string lpDestFolder, string zipFileName,
        bool deleteTempFile, bool writeFiles, string addExtension, Hashtable htFiles, int bufferSize)
    {
        ...
        string lpFilePath = string.Empty;
                ...
                while (fsTemp.Position != fsTemp.Length)
                {
                    ...
                    if (!string.IsNullOrEmpty(line))
                    {
                        ...
                        if (gzf.ParseFileInfo(line) && gzf.Length > 0)
                        {
                            ...
                            lpFolder = Path.GetDirectoryName(lpFilePath);
