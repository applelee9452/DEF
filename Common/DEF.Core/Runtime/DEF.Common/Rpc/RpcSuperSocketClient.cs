...
namespace DEF
{
    public class RpcSuperSocketClient
    {
        public Action<string> ActionSocketSessioned { get; set; }
        public Action ActionSocketConnected { get; set; }
        public Action ActionSocketConnectFailed { get; set; }
        public Action ActionSocketClosed { get; set; }
        public Action<SocketErrorEventArgs> ActionSocketError { get; set; }
        NetworkClientSuperSocket MbTcpClient { get; set; }
        ...
        public RpcSuperSocketClient(IClient client, string host, int port)
        {
            Client = client;
            Host = host;
            Port = port;
        ...
        public void Connect()
        {
            if (MbTcpClient != null)
            {
                GameObject.Destroy(MbTcpClient.gameObject);
                MbTcpClient = null;
            }
            ...
            if (MbTcpClient == null)
            {
                ...
                var go_networktcpclient = new GameObject("NetworkTcpClient");
                go_networktcpclient.transform.parent = go_def.transform;

                MbTcpClient = go_networktcpclient.GetComponent<NetworkClientSuperSocket>();
                if (MbTcpClient == null)
                {
                    MbTcpClient = go_networktcpclient.AddComponent<NetworkClientSuperSocket>();
                }
