...
namespace NPBehave
{
    public class Clock
    {
        class Timer
        {
            ...
            public void ScheduleAbsoluteTime(System.Random rd, double elapsedTime)
            {
                scheduledTime = elapsedTime + delay - randomVariance * 0.5f + randomVariance * rd.NextDouble();
            }
        }

        Behave Behave;
        private List<System.Action> updateObservers = new();
        private Dictionary<System.Action, Timer> timers = new();
        private HashSet<System.Action> removeObservers = new();
        private HashSet<System.Action> addObservers = new();
        private HashSet<System.Action> removeTimers = new();
        private Dictionary<System.Action, Timer> addTimers = new();
        private bool isInUpdate = false;
        private double elapsedTime = 0f;
        private List<Timer> timerPool = new();
        private int currentTimerPoolIndex = 0;

        public Clock(Behave behave)
        {
            Behave = behave;
        }
        ...
        public void AddTimer(float delay, float randomVariance, int repeat, System.Action action)
        {
            ...
            timer.repeat = repeat;
            timer.ScheduleAbsoluteTime(Behave.Rd, elapsedTime);
        }
