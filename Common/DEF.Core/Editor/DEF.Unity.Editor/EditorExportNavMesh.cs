...
public class EditorExportNavMesh : Editor
{
    ...
    private static List<Vert> vertList = new();
    private static List<Face> faceList = new();
    private static List<Pair> pairList = new();
    private static Dictionary<Vert, Face> vertFaceDict = new();
    private static Dictionary<Vert, Dictionary<Vert, Pair>> vertPairDict = new();
    private static Dictionary<float, Dictionary<float, Vert>> pointVertDict = new();
    private static Dictionary<int, Vert> indexVertDict = new();
    private static string outputClientFolder = "../Data/Data/NavMesh/";
    private static string outputServerFolder = "../Data/Data/NavMesh/";
    ...
    public static void Export()
    {
        ...
        {
            Debug.LogError($"导出NavMesh Error，场景里没有需要被导出的物体，请先用NavMesh进行Bake。");
            return;
        }
        ...
        var geom = LoadInputMesh();
        var navMesh = BuildNavMesh(geom);
        SaveNavMesh(navMesh);
        ...
        Debug.Log($"导出NavMesh Result，Vertices:[{vertList.Count}] - Faces:[{faceList.Count}]");
    }
    ...
    //private static DtNavMesh ReadRecastObjFile()
    //{
    //    var filename = SceneManager.GetActiveScene().name;
    //    var path = outputClientFolder + filename + ".obj";
    ...
    private static DemoInputGeomProvider LoadInputMesh()
    {
        var filename = SceneManager.GetActiveScene().name;
        var path = outputClientFolder + filename + ".obj";
        ...
        DemoInputGeomProvider geom = DemoObjImporter.Load(buffer);
        return geom;
    }

    private static DtNavMesh BuildNavMesh(DemoInputGeomProvider geom)
    {
        var settings = new RcNavMeshBuildSettings();
        SoloNavMeshBuilder soloNavMeshBuilder = new SoloNavMeshBuilder();
        var  buildResult = soloNavMeshBuilder.Build(geom, settings);
        return buildResult.NavMesh;
    }

    private static void SaveNavMesh(DtNavMesh navMesh)
    {
        var filename = SceneManager.GetActiveScene().name;
        var navmeshFilePath = outputClientFolder +  $"{filename}.navmesh";
    ...
    private static void CopyObjFiles()
    {
        ...
        // *.bytes, *.obj, *_internal.obj文件不再拷贝到服务器的Config/Navmesh目录下，不再需要了，减少服务器数据文件的大小
        //得到原文件根目录下的所有文件
        //	    {
