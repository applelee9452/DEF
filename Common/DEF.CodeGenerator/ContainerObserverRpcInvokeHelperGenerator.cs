...
{
    [Generator]
    public class ContainerObserverRpcInvokeHelperGenerator : ISourceGenerator
    {
...
#endif
            context.RegisterForSyntaxNotifications(() => new ClassesWithInterfacesReceiver("DEF.IContainerRpcObserver"));
        }
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var i in classes)
            {
                ...
                foreach (var iface in intefaces)
                {
                    ...
                    {
                        if (attr.AttributeClass.Name == "ContainerRpcObserverAttribute")
                        {
                            var s = attr.ToString();
                            int i1 = s.IndexOf('\"');
            ...
            foreach (var i in map_interface)
            {
                ...
                foreach (var m in members)
                {
                    ...
                    sb_methods.Append($@"(int int_st, IContainerRpcObserver instance, byte[] method_data)");
...
[RegisterContainerRpcObserverInvokeHelper(""{container_type}"")]
