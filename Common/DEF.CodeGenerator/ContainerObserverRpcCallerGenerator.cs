...
{
    [Generator]
    public class ContainerObserverRpcCallerGenerator : ISourceGenerator
    {
...
#endif
            context.RegisterForSyntaxNotifications(() => new DerivedInterfacesReceiver("DEF.IContainerRpcObserver"));
        }
...
public class {factory_name} : ContainerRpcObserverCallerFactory
{{
    ...
    public override ContainerRpcObserverAttribute GetContainerRpcObserverAttribute()
    {{
        var t = typeof({interface_name});
        return DEFUtils.GetContainerRpcObserverAttribute(t);
    }}

    public override IContainerRpcObserver CreateContainerRpcObserver(IRpcInfo rpcinfo, IRpcer rpcer)
    {{
