...
public abstract class ContainerStateful : ContainerBase
{
    public string ContainerType { get; private set; }
    public string ContainerId { get; private set; }
    protected GrainContainerStateful Grain { get; set; }

    public void Setup(ILogger logger, IService service, GrainContainerStateful grain, IGrainFactory grain_factory, string container_type, string container_id)
    {
...
public abstract class ContainerStatefulFactory
{
    public abstract ContainerStateful Create(ILogger logger, IService service, GrainContainerStateful grain,
        IGrainFactory grain_factory, string container_type, string container_id);
}

public class ContainerStatefulFactory<T> : ContainerStatefulFactory where T : ContainerStateful, new()
{
    public override ContainerStateful Create(ILogger logger, IService service, GrainContainerStateful grain,
        IGrainFactory grain_factory, string container_type, string container_id)
    {
        var container = new T();
